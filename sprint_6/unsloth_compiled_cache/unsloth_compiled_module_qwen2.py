...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.qwen2.modeling_qwen2 import (__name__, Callable, Optional, torch, nn, ACT2FN, Cache, GenerationMixin, use_kernel_func_from_hub, FlashAttentionKwargs, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, maybe_autocast, Qwen2Config, Qwen2PreTrainedModel, Qwen2Model, Qwen2ForCausalLM)
...
class Qwen2MLP(nn.Module):
    ...
    def forward(self, x):
        return Qwen2MLP_forward(self, x=x)


@torch.compile(fullgraph = False, dynamic = True, options = torch_compile_options)
@torch.no_grad()
@dynamic_rope_update  # power user: used with advanced RoPE types (e.g. dynamic rope)
def Qwen2RotaryEmbedding_forward(self, x, position_ids):
...
class Qwen2RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return Qwen2RotaryEmbedding_forward(self, x=x, position_ids=position_ids)
...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return Qwen2Attention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, cache_position=cache_position, **kwargs)
...
class Qwen2RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return Qwen2RMSNorm_forward(self, hidden_states=hidden_states)
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return Qwen2ForCausalLM_forward(self, input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, labels=labels, use_cache=use_cache, cache_position=cache_position, logits_to_keep=logits_to_keep, **kwargs)
